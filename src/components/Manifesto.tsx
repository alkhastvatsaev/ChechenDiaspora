...
export default function Manifesto() {
  return (
    <div className="flex flex-col w-full bg-kherch-dark text-vainakh-stone min-h-full">
      {/* Hero Header */}
      <motion.div 
      ...
      >
        <div className="absolute top-0 inset-x-0 h-32 bg-gradient-to-b from-hearth-amber/10 to-transparent opacity-50" />
        <div className="relative inline-block mb-2">
           <div className="absolute -inset-4 bg-hearth-amber/20 blur-2xl rounded-full animate-pulse-slow"></div>
           <Flame size={48} className="relative text-hearth-amber mx-auto" />
        </div>
        <h1 className="text-4xl font-black tracking-tighter uppercase leading-none">
          Manifeste de la <span className="text-hearth-amber">Diaspora</span>
        </h1>
        <p className="text-xl font-bold text-vainakh-stone/60 italic tracking-tight">
          Le Fardeau et l&apos;Excellence.
        </p>
        <div className="w-16 h-1 bg-hearth-amber/30 mx-auto rounded-full mt-6"></div>
      </motion.div>

      {/* Intro Text */}
      <div className="px-8 mb-12">
        <div className="p-8 bg-white/5 rounded-[2.5rem] border border-white/10 backdrop-blur-md">
          <p className="text-lg font-medium leading-relaxed text-vainakh-stone/80 text-center">
      ...
      <div className="px-8 pb-32 space-y-8">
        {MANIFESTO_SECTIONS.map((section, idx) => (
          ...
          >
            <div className="absolute -left-4 -top-4 text-6xl font-black text-white/[0.03] select-none group-hover:text-hearth-amber/[0.05] transition-colors duration-700">
              {section.id}
            </div>
            
            <div className="bg-white/5 rounded-[3rem] p-8 border border-white/5 backdrop-blur-sm hover:border-white/10 transition-all duration-500">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center border border-white/5">
                  {section.icon}
                </div>
                <div>
                  <div className="text-[10px] font-black uppercase tracking-[0.3em] text-hearth-amber/70 mb-1">{section.subtitle}</div>
                  <h3 className="text-2xl font-black tracking-tight leading-none uppercase">{section.title}</h3>
                </div>
              </div>

              <div className="space-y-4">
                <p className="text-base text-vainakh-stone/70 leading-relaxed font-medium">
                  {section.content}
                </p>
                {section.content2 && (
                  <p className="text-base text-vainakh-stone/70 leading-relaxed font-medium">
                ...
                {section.highlight && (
                   <div className="p-6 bg-rose-500/10 rounded-3xl border border-rose-500/20 mt-6">
                      <h4 className="text-rose-400 font-black text-sm uppercase mb-2 tracking-tight">{section.highlight}</h4>
                ...
                {section.quote && (
                  <div className="pt-6 mt-6 border-t border-white/10 italic text-xl font-bold text-hearth-amber/90 leading-tight">
                    {section.quote}
                  </div>
                )}
                  ...
                  <div className="space-y-4 pt-4">
                    {section.list.map((item, i) => (
                      <div key={i} className="flex gap-4 items-start">
                         <div className="w-6 h-6 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-400 text-xs shrink-0 mt-1">
                            <ChevronRight size={14} />
                         </div>
                         <p className="text-sm font-bold text-vainakh-stone/80">{item}</p>
                      </div>
                    ))}
                  </div>
                )}
        ...
        >
          <div className="w-20 h-0.5 bg-hearth-amber/20 mx-auto rounded-full"></div>
          <p className="text-2xl font-black tracking-tight leading-relaxed max-w-sm mx-auto uppercase italic text-hearth-amber/80">
            &quot;Personne ne viendra nous sauver, si ce n&apos;est nous-mêmes, par la grâce de Dieu.&quot;
          </p>
          <div className="space-y-4">
            <p className="text-vainakh-stone/50 font-medium leading-loose max-w-lg mx-auto">
            ...
            <p className="text-lg font-black tracking-widest text-hearth-amber uppercase mt-12">
